...
export async function getStaticProps({ preview, params, locale }: GetStaticPropsContext<{ pages: string[] }>) {
    ...
    const data = pageItem && (await getPage({ variables: { id: pageItem.id! }, config, preview }));
...
export default function Pages({ page }: InferGetStaticPropsType<typeof getStaticProps>) {
    return <div className="max-w-2xl mx-auto py-20">{page?.body && <Text html={page.body} />}</div>;
}
